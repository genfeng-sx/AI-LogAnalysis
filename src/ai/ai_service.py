...
class AIServiceBase(ABC):
    ...
    @abstractmethod
    def analyze_log(self, log_content, alert_content=None, custom_prompt=None, conversation_history=None):
        """分析日志内容
    ...
    def analyze_log(self, log_content, alert_content=None, custom_prompt=None, conversation_history=None):
        ...
        try:
            ...
            try:
                ...
                # 如果仍无法解析JSON，则返回原始文本形式
                return {
                    "is_threat": False,
                    "confidence": 0,
                    "analysis": content,
                    "recommendations": "AI返回的内容不是JSON格式，但已显示原始回答。"
                }
...
class LocalAIService(GenericAIService):
    ...
    def analyze_log(self, log_content, alert_content=None, custom_prompt=None, conversation_history=None):
        ...
        try:
            # 调用父类的方法
            result = super().analyze_log(log_content, alert_content, custom_prompt, conversation_history)
