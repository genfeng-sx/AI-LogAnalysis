...
class SettingsDialog(QDialog):
    ...
    def init_ui(self):
        ...
        # IP掩码开关
        self.ip_mask_checkbox = QCheckBox("启用IP地址掩码")
        general_form.addRow("", self.ip_mask_checkbox)

        # 添加IP映射显示选项
        self.show_ip_mapping_checkbox = QCheckBox("在主界面显示IP映射")
        general_form.addRow("", self.show_ip_mapping_checkbox)
...
{alert_content} - 告警内容（如果有）""")
    ...
    def load_settings(self):
        ...
        self.show_ip_mapping_checkbox.setChecked(self.config_manager.get("show_ip_mapping", False))
    ...
    def save_settings(self):
        ...
        self.config_manager.set("show_ip_mapping", self.show_ip_mapping_checkbox.isChecked())
        ...
        for key, info in self.api_services.items():
            if key in ["deepseek", "doubao", "local", "other"]:
